...
var TreoStorage = (treoStorage && typeof treoStorage === 'object' && 'default' in treoStorage ? treoStorage['default'] : treoStorage);
...
var TaxiController = (function (SiftController) {
  function TaxiController() {
  ...
  if ( SiftController ) TaxiController.__proto__ = SiftController;
  TaxiController.prototype = Object.create( SiftController && SiftController.prototype );
  TaxiController.prototype.constructor = TaxiController;
  ...
  TaxiController.prototype.loadView = function loadView (state) {
    var this$1 = this;
    ...
    var data = this.storage.getUser({ keys: ['currency']}).then(function (results) {
      if (results && results[0] && results[0].value) {
        this$1._currency = results[0].value;
      }
      return this$1.getData(this$1._detail);
    });
    return {html: 'view.html', data: data};
  };

  TaxiController.prototype.onStorageUpdate = function onStorageUpdate (value) {
    var this$1 = this;

    console.log('sift-taxi: onStorageUpdate: ', value);
    this.getData(this._detail).then(function (data) {
      this$1.publish('storageupdate', data);
    });
  };

  TaxiController.prototype.onCurrencyChange = function onCurrencyChange (value) {
    var this$1 = this;

    console.log('sift-taxi: onCurrencyChange: ', value);
    this.storage.get({ bucket: '_redsift', keys: ['webhooks/settings-wh'] }).then(function (result) {
      console.log('sift-taxi: onCurrencyChange webhook url: ', result[0].value);
      this$1._currency = value;
      this$1.storage.putUser({ kvs: [{ key: 'currency', value: value }] });
  ...
  TaxiController.prototype.getData = function getData (details) {
    var this$1 = this;
    ...
    return Promise.all(ps).then(function (results) {
      ...
      ret.currency = this$1._currency;
      return ret;
    });
  };

  TaxiController.prototype._getTodayData = function _getTodayData () {
    return this.storage.get({
      bucket: 'day',
      keys: [moment$1().utc().valueOf().toString()]
    });
  };

  TaxiController.prototype._getYTDData = function _getYTDData () {
    return this.storage.get({
      bucket: 'year',
      keys: ['' + moment$1().year()]
    });
  ...
  TaxiController.prototype._getMonthData = function _getMonthData (details) {
    var this$1 = this;

    var cKeys = this._getChartKeys(this._sizeClass.width, details);
    return this.storage.get({ bucket: 'month', keys: cKeys }).then(function (results) {
      ...
      var lines = results.map(function (r) {
        ...
        if(r.value) {
          ...
          jval.companies.forEach(function (v, i) {
            var o = (i===0)?0:2;
            ms.v[o*i] += v;
            pt[i] += v;
          });
          ...
          if(k === moment$1().utc().startOf('month').valueOf()) {
            stats.mtd = jval.total;
          }
        }
        if(details && details.month === k) {
          ms.v[this$1.highlightIndexes[details.company]] = parseInt(details.total);
        }
        return ms;
      });
      if (stats.trips > 0) {
  ...
  TaxiController.prototype._getChartKeys = function _getChartKeys (width, details) {
    var num = 6;
    ...
    for (var i = 0; i < num; i++) {
      if (details) {
        chartKeys.unshift(moment$1.utc(parseInt(details.month)).subtract(i, 'months').valueOf().toString());
      }
      else {
        chartKeys.unshift(moment$1().utc().startOf('month').subtract(i, 'months').valueOf().toString());
      }
    }
    return chartKeys;
  };
