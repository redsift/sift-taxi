...
function lines(id) {
  ...
  var tid = null;
  if (id) tid = 'tip-' + id;
  var rtip = tip(tid).offset([- DEFAULT_TIP_CIRCLE_SIZE - DEFAULT_TIP_OFFSET, 0]);
...
//      pattern.background('transparent');      
      ...
      if (legend.length > 0 && legendOrientation !== 'voronoi') {
        lchart = svg$1().width(w).height(h).inset(0).fill(colors).theme(theme).orientation(legendOrientation);
      ...
      hLabel
        .attr('text-anchor', function (d) { return d.v[1] == null ? 'start' : 'middle'; })
...
var TaxiSift = (function (SiftView) {
  ...
  TaxiSift.prototype.onLoad = function onLoad () {
    ...
    for (var i = 0; i < radios.length; i++) {
      ...
      r.addEventListener('click', function (ev) {
        ...
        this$1.publish('currency', ev.target.id);
      }, false);
    }
  };
   ...
   */
  TaxiSift.prototype.onStorageUpdate = function onStorageUpdate (data) {
   ...
   */
  TaxiSift.prototype._updateSections = function _updateSections (height) {
    var show = 'none';
   ...
   */
  TaxiSift.prototype._updateCurrencySelector = function _updateCurrencySelector (code) {
   ...
   */
  TaxiSift.prototype._updateStats = function _updateStats (currency, stats) {
    ...
    if(!this._stack) {
      ...
      this._stack = lines('stacked')
        ...
        .tipHtml(function (d, i, s) {
          if(s < 0) return;
          return (d.v[s] === 0)?d.v[s]:this$1.currencySymbols[this$1._data.currency]+d.v[s].toFixed(2);
        })
        .tickDisplayValue(function (d) { return (d === 0)?d:this$1.currencySymbols[this$1._data.currency]+d; })
    ...
    if(!this._pie) {
      ...
      this._pie = pies()
        .fill([
          ...
          this.fill.hailo.color,
          this.fill.addisonlee.color])
        .displayValue(function (v) { return (100 * v / this$1._data.stats.total).toFixed(0) + '%'; });
    }
